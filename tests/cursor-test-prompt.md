# Cursor Test Prompt for SAPUI5 LLM-Ready Validation

## Instructions

Copy and paste this prompt into Cursor's chat panel (Cmd+L) to test the SAPUI5 LLM-Ready design system.
...
1. Copy the JSON output from Cursor
2. Save it as `test-outputs/cursor-output.json`
3. Run validation:
```bash
...
If Cursor outputs code instead of JSON, add this to the prompt:
```
...
If Cursor invents components, add this to the prompt:
```
